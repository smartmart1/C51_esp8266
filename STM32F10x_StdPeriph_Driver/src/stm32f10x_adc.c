...
void ADC_Init(ADC_TypeDef* ADCx, ADC_InitTypeDef* ADC_InitStruct)
{
  ...
  /* SetValue DUALMOD bits according to ADC_Mode value */
  /* SetValue SCAN bit according to ADC_ScanConvMode value */
  ...
  /* SetValue ALIGN bit according to ADC_DataAlign value */
  /* SetValue EXTSEL bits according to ADC_ExternalTrigConv value */
  /* SetValue CONT bit according to ADC_ContinuousConvMode value */
  ...
  /* SetValue L bits according to ADC_NbrOfChannel value */
...
void ADC_Cmd(ADC_TypeDef* ADCx, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    /* SetValue the ADON bit to wake up the ADC from power down mode */
...
void ADC_DiscModeChannelCountConfig(ADC_TypeDef* ADCx, uint8_t Number)
{
  ...
  /* SetValue the discontinuous mode channel count */
  tmpreg2 = Number - 1;
  tmpreg1 |= tmpreg2 << 13;
...
void ADC_RegularChannelConfig(ADC_TypeDef* ADCx, uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
  ...
  if (ADC_Channel > ADC_Channel_9)
  {
    ...
    /* SetValue the new channel sample time */
    tmpreg1 |= tmpreg2;
    /* Store the new register value */
    ADCx->SMPR1 = tmpreg1;
  }
  ...
  if (Rank < 7)
  {
    ...
    /* SetValue the SQx bits for the selected rank */
    tmpreg1 |= tmpreg2;
    /* Store the new register value */
    ADCx->SQR3 = tmpreg1;
  }
...
void ADC_ExternalTrigInjectedConvConfig(ADC_TypeDef* ADCx, uint32_t ADC_ExternalTrigInjecConv)
{
  ...
  /* SetValue the external event selection for injected group */
...
void ADC_InjectedChannelConfig(ADC_TypeDef* ADCx, uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
  ...
  /* SetValue the JSQx bits for the selected rank */
  tmpreg1 |= tmpreg2;
  /* Store the new register value */
  ADCx->JSQR = tmpreg1;
}
...
void ADC_InjectedSequencerLengthConfig(ADC_TypeDef* ADCx, uint8_t Length)
{
  ...
  /* SetValue the injected sequnence lenght JL bits */
  tmpreg2 = Length - 1;
  tmpreg1 |= tmpreg2 << 20;
  /* Store the new register value */
  ADCx->JSQR = tmpreg1;
...
/**
  * @brief  SetValue the injected channels conversion value offset
  * @param  ADCx: where x can be 1, 2 or 3 to select the ADC peripheral.
...
void ADC_SetInjectedOffset(ADC_TypeDef* ADCx, uint8_t ADC_InjectedChannel, uint16_t Offset)
{
  ...
  /* SetValue the selected injected channel data offset */
...
void ADC_AnalogWatchdogCmd(ADC_TypeDef* ADCx, uint32_t ADC_AnalogWatchdog)
{
  ...
  /* SetValue the analog watchdog enable mode */
                                        ...
                                        uint16_t LowThreshold)
{
  ...
  /* SetValue the ADCx high threshold */
  ADCx->HTR = HighThreshold;
  /* SetValue the ADCx low threshold */
...
void ADC_AnalogWatchdogSingleChannelConfig(ADC_TypeDef* ADCx, uint8_t ADC_Channel)
{
  ...
  /* SetValue the Analog watchdog channel */
  tmpreg |= ADC_Channel;
  /* Store the new register value */
  ADCx->CR1 = tmpreg;
}
